...
TEST_SUITE(qjs_tests) {
    ...
    TEST_CASE(runtime_context_and_eval_cover_success_and_error_paths) {
        auto f = [&]() {
            ...
            EXPECT_TRUE(wrapped_global.as<qjs::Object>().get_property("from_cpp").as<int64_t>() ==
                        9);

            auto atom = qjs::Atom(default_ctx.js_context(),
                                  JS_NewAtom(default_ctx.js_context(), "qjs-test"));
    ...
    TEST_CASE(function_wrappers_cover_cpp_js_and_raw_function_invocation) {
        auto f = [&]() {
            ...
            auto make_object = qjs::Function<qjs::Object()>::from(ctx.js_context(), [&ctx]() {
                return ctx.eval("({ kind: 'native-object' })", "<eval>", eval_flags)
                    .as<qjs::Object>();
            });
            auto add_one =
    ...
    TEST_CASE(function_wrappers_cover_lvalue_functor_storage) {
        ...
        auto lvalue_functor =
            qjs::Function<int64_t(int64_t)>::from(ctx.js_context(), counting_functor);
