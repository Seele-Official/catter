...
Compiler identify_compiler(std::string_view compiler_name){
    ...
    static const auto DEFAULT_PATTERNS = []() {
        
        auto create_compiler_regex = [](std::string_view base_pattern, bool with_version) -> std::regex {
            ...
            std::string full_pattern = "^" + optional_path_prefix + pattern_with_version + exe_suffix + "$";
        ...
        return std::to_array<CompilerPattern>({
            ...
            // GCC pattern
            {Compiler::gcc, create_compiler_regex(R"((?:[^/]*-)?(?:gcc|g\+\+|gfortran|egfortran|f95))", true)},
            // GCC internal executables pattern: matches GCC's internal compiler phases
            {Compiler::gcc, create_compiler_regex(R"((?:cc1(?:plus|obj|objplus)?|f951|collect2|lto1))", false)},
