...
// Concept to check if a type is a ServiceFactory, which is defined as any invocable that returns a std::unique_ptr to a type derived from ipc::Service
template <typename ServiceFactoryType>
concept ServiceFactoryLike = std::invocable<ServiceFactoryType> && ServiceFactoryLike_helper<std::invoke_result_t<ServiceFactoryType>>::value;
...
public:
    ...
    void run(std::vector<std::string> shell, ServiceFactoryType&& factory) {
        ...
        this->run_(std::move(shell), FactoryWrapper{.factory = std::forward<ServiceFactoryType>(factory)});
    }
...
private:
    void run_(std::vector<std::string> shell, ServiceFactory factory) {
        ...
        if(!acc_ret) {
            throw std::runtime_error(std::format("Failed to create acceptor: {}", acc_ret.error().message()));
        }
    ...
    eventide::task<int64_t> spawn(std::vector<std::string> shell) {
        ...
        std::string exe_path =
            (util::get_catter_root_path() / config::proxy::EXE_NAME).string();
